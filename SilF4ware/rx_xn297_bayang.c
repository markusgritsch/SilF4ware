...
// packet period in uS
#define PACKET_PERIOD ( 3000 / WALLTIME_CORRECTION_FACTOR )
...
bool checkrx( void )
{
	if ( LOOPTIME < 500 ) { // Kludge. 500 seems to work better for telemetry, so we skip some invocations accordingly.
		static int count;
		if ( count == 0 ) {
